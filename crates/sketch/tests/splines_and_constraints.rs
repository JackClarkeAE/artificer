...
use artificer_sketch::{
    ConfirmationSource, EvaluatedCurve2, PointInput, SketchConstraintKind,
    SketchDefinition, SketchPoint2, SketchRecipe, build_arrangement, compile_selected_profile,
    intersect_curves,
};
...
fn fit_point_spline_recipe_interpolates_authored_fit_points() {
    ...
    let transaction = sketch.stage(recipe, "Fit Spline").expect("stage fit spline");
    sketch
    ...
    let start = sketch.evaluated_curve(curve_id).unwrap().evaluate(0.0).unwrap();
    let end = sketch.evaluated_curve(curve_id).unwrap().evaluate(1.0).unwrap();
...
fn spline_line_and_spline_spline_intersection() {
    ...
    let s1 = sketch1.evaluated_curve(sketch1.active_entities().next().unwrap().id).unwrap();
...
fn tangent_constraint_and_collinear_constraint_solving() {
    let precision = PrecisionPolicy::default();
    let mut sketch = SketchDefinition::new();
    let t1 = sketch.stage(SketchRecipe::Point { position: SketchPoint2::new(0.0, 0.0) }, "P1").unwrap();
    sketch.commit(t1, ConfirmationSource::GreenTick).unwrap();
    let t2 = sketch.stage(SketchRecipe::Point { position: SketchPoint2::new(5.0, 1.0) }, "P2").unwrap();
    sketch.commit(t2, ConfirmationSource::GreenTick).unwrap();
    let t3 = sketch.stage(SketchRecipe::Point { position: SketchPoint2::new(10.0, 2.0) }, "P3").unwrap();
    sketch.commit(t3, ConfirmationSource::GreenTick).unwrap();
    let t4 = sketch.stage(SketchRecipe::Point { position: SketchPoint2::new(15.0, 5.0) }, "P4").unwrap();
...
fn bspline_curves_compile_to_planar_profile() {
    ...
    let compiled = compile_selected_profile(&arrangement, std::slice::from_ref(signature), &precision).unwrap();
    assert_eq!(compiled.profile.regions.len(), 1);
    assert_eq!(compiled.profile.regions[0].outer.curves.len(), 2);

    assert!(compiled.profile.regions[0].outer.curves.iter().any(|c| matches!(c, PlanarCurve2::Bspline { .. })));
    assert!(compiled.profile.regions[0].outer.curves.iter().any(|c| matches!(c, PlanarCurve2::Line { .. })));
}
