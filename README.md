...
- **A review of the kernel's edge cases.** A pass over the kernel for wrong answers, missed refusals and slow paths. Most of what it fixed used to publish a wrong solid without a word. A cut or boss sketched on a frame that faces into the body now lands where it was drawn. A prism tool standing on a box, or missing it, leaves the box whole. Pushing a top down past a pocket underneath is refused. A cut stopping a hair short of the far face goes through instead of leaving a film of a floor. A fillet on a concave edge is refused by name, where the faceted tier used to publish the body unchanged, and shelling a turned body opens only the cap you picked. Revolve profiles are checked as extrusion profiles are, and one that touches the axis at a single point is refused rather than crashing. A sweep whose path runs back through itself is refused. A whole-circle profile sweeps round a twisting path, and lofts far from the origin build as they do at it. Booleans far out along a plane crossing now work, and mirrors and patterns keep to the coordinate limit. Scripts have firm limits on depth and work (listed in the scripting guide) instead of crashing, redo keeps its history, arcs save and reload to the same angle, and `b.faces()` names every face a step made. Two slow paths got faster: edge matching in the faceted Boolean takes about half the time on the facet-permutation test, and the exact Boolean reads each face's region once instead of once per pair. Five new refusal codes, each with its reason, are listed in the report schema.
