...
- **Fillets on concave edges, exactly.** A boss standing on its plate, a pocket's floor, a counterbore's floor rim: the rim fillets and chamfers by a torus or cone band built in place, with its material added, and the volume checked against Pappus. A concave straight edge on any planar body (a drilled block with a step, an L with a pocket) fills by its own corner region swept along the edge and unioned in, certified against the closed form. A selection can mix concave and convex edges and is finished in one call. A fillet whose radius changes along a straight edge lands too, as flat facets lofted between its two end sections, labelled approximate with the sagitta it carries. The old refusal of a reflex edge is retired for planar bodies.
- **Sheet bodies.** A sketch chain can be extruded or revolved into a surface with no caps, a profile can be patched into a planar sheet, sheets can be stitched into a closed solid (or refused with the gap measured when an edge falls outside the precision), thickened into a solid with exact offsets of planes, cylinders, cones, spheres and tori, and trimmed by a plane. Spline chains extrude into B-spline sheets, and a B-spline sheet thickens by an approximate offset that reports its deviation. Sheets export to STEP as open shells and are scriptable with `surface_extrude`, `surface_revolve`, `patch`, `stitch`, `thicken` and `trim`.
- **Booleans between curved faces.** Cones, spheres and tori are now in the analytic engine's vocabulary, so a sphere seated on a plate, a counterbore into a drafted boss, or a bore that clears a torus blend combine exactly and unlabelled, and sections are closed by the cells of their arrangement rather than refused. Where two curved faces meet along a curve the kernel has no closed form for (a hole through a torus band, a sphere cut by an off-centre bore, two tori crossing, a cone cut by an oblique plane), a new numerical rung sits between the exact engine and the faceted tier: the crossing is marched and fitted as a B-spline edge to within 5e-10, the faces stay analytic, the volume is still integrated exactly, and the step is labelled approximate with the measured deviation. Every such Boolean conserves volume to 1e-7. Sweeps and lofts with ruled or B-spline walls still fall to the faceted tier and say so.
- **STEP import.** Open a STEP file (ISO 10303-21) into an exact body. Planes, cylinders, cones, spheres, ring tori and non-rational B-spline surfaces bounded by their iso-lines, with lines, circles, ellipses and B-spline curves, are read into the kernel's own vocabulary and conformed to its conventions: a seamed 360° face becomes the kernel's two halves, an inch file lands in millimetres, a rational circle snaps to a circle. Everything the kernel exports round-trips to 1e-9 in volume and area with the same faces. A file with a face the kernel cannot read opens as a reference mesh on the faceted tier with every refusal named beside it. Reachable from a script (`import_step`), the API and the JSON-RPC server.
- **Robustness at scale.** A uniform grid over face extents prunes the pairwise passes of the exact Boolean, the profile Boolean's crossing pass is sub-quadratic through a segment grid, and sewing welds through hash grids instead of linear scans; small bodies and every existing fixture are byte-identical. The wire ceilings on a profile rise from 1,024 curves and 128 loops to 16,384 and 1,024. The faceted tier's cap is a precision-driven budget that refuses by name and can be cancelled. One agreement model (`Agreement`) replaces a spread of tolerance formulas, and a scale-invariance test pins the kernel's answers at 10⁻³, 1 and 10³ and 10⁵ mm from the origin. Benches over 10² and 10³ faces are in the tree; the biggest remaining costs are named in the ADR.
- **A CAM tab.** Press Auto-CAM and the active body is recognised as turned or milled, stock and tools are chosen from a library seeded into your data folder, the operations are planned (face, rough, drill, bore, finish, groove, back corner, part off on the lathe; face, pocket, profile, drill on the mill), G-code is posted in the LinuxCNC dialect and read back through the tab's own interpreter, and the simulation plays: a semi-transparent tool taking material off a semi-transparent billet until the stock is the part, with rapids and cuts in two colours and collisions in red. On the lathe the stock is the exact (r, z) section, so the final stock equals the part's section to 1e-9. On the mill the stock is a heightmap. Setup, operations and simulation live on the card; the plan is staged behind the same confirm gate as every feature and exports as `.ngc`.
- **A Simulation tab.** A structural study: fix faces, load faces or apply a pressure, add the part's own weight, choose a material and a resolution, and solve. Stress is painted on the part with a legend in MPa, deflection can be exaggerated up to 100×, and the card reports the safety factor, the peak stress and deflection, the voxel count, and how much the answer moved between resolutions. A cantilever lands within 1.1% of the beam formula at the finest resolution, and a plate with a hole shows the textbook stress concentration. A thermal study on the same grid paints temperature. A motion timeline scrubs and plays a mechanism and measures clearance at every frame, flagging the first collision. Topology optimisation, marked experimental, melts material away over iterations. Every result is a voxel approximation and the card says so.

## What is new in 0.99.81

A correctness release for 0.99.8: a review of the kernel's edge cases, looking for wrong answers, missing refusals and slow paths.

- **Wrong solids that used to be published without a word.**
  - A cut or boss sketched on a frame that faces into the body now lands where it was drawn.
  - A prism tool standing on a box, or missing it, leaves the box whole.
  - Pushing a top face down past a pocket underneath is refused.
  - A cut that stops a hair short of the far face goes through instead of leaving a film of a floor.
  - A fillet on a concave edge is refused by name. The faceted tier used to publish the body unchanged.
  - Shelling a turned body opens only the cap you picked.
- **Revolve, sweep and loft.**
  - Revolve profiles are checked the way extrusion profiles are.
  - A profile that touches the axis at a single point is refused instead of crashing.
  - A sweep whose path runs back through itself is refused.
  - A whole-circle profile sweeps round a twisting path.
  - Lofts far from the origin build as they do at it.
- **Booleans, mirrors and patterns.**
  - Booleans far out along a plane crossing now work.
  - Mirrors and patterns stay within the coordinate limit.
  - Edge finishes on boxes, U shapes and hole rims take only their own corner.
- **Scripts.**
  - Scripts have firm limits on nesting depth and total work, listed in the scripting guide, instead of crashing.
  - Redo keeps its history.
  - Arcs save and reload to the same angle.
  - `b.faces()` names every face a step made.
- **Faster.**
  - Edge matching in the faceted Boolean takes about half the time on the facet-permutation test.
  - The exact Boolean reads each face's region once instead of once per pair of faces.
- Five new refusal codes, each with its reason, are listed in the report schema.

## What is new in 0.99.8
...
- **Revolve, Axis and Sweep.** Revolve is now a history feature. It turns a sketch's profile about a centreline, the sketch's own axes, an origin axis, or a new construction axis. It can make a full turn or turn through an angle typed in, or written over a variable, one way, the other way or both ways. It builds a new body, adds to one or cuts from one. The Axis command places a construction axis along a straight edge, through a curved face's centre, or where two planes meet, and it follows them when the model rebuilds. Sweep carries a sketch's profile along a path drawn in another sketch: lines, arcs and splines joined end to end and tangent where they meet. By default the profile turns with the path, twisting as little as possible, or it can keep the orientation it was drawn in. A straight path gives an exact solid, and so does a single arc whose axis lies in the profile's plane. Any other path is skinned through copies of the profile, added where the skin strays, until it lies within 0.00001 mm of the true sweep; the result states the gap it met. A skinned sweep can be added to or cut from a body as well, for a bent pipe through a block for example. It goes through the faceted Boolean, so that result is an approximation and says so. Each feature has a chip that reopens its editor. ADR 0055 records the design.
- **Revolve, finished.** A triangle turned about one of its legs now makes a pointed cone. A profile with holes in it revolves, each hole becoming a cavity (or a channel, on a partial turn), and a sketch with several regions revolves into several solids. A revolve turned about the body's own axis now adds and cuts exactly whatever its faces are: a V-groove cut into a shaft has true conical flanks, a ball on a shaft end is a true sphere, and a round groove is a true torus. The two bodies are combined in their shared half-section and turned again (ADR 0026 F4). The open shell of a cone body is exact for the same reason. A revolve through less than a full turn can have its rims filleted and chamfered, and can be shelled, keeping a wall along each flat end face that bends round the axis past half a turn. Up to half a turn its end faces can be opened too, which gives a cutaway. On the card, **Pick in view** takes the axis from a sketch line, a construction axis or a straight edge of the model; an edge becomes a construction axis of its own. Construction axes now have their own section in the Browser, with a menu to edit, rename, hide or delete them, and scripts can name one with `axis(...)`. A cone, sphere or torus turned about a *different* axis than the body's still combines through the faceted Boolean, and says so.
