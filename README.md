...
- **Sketch constraints from the canvas.** Coincident, horizontal, vertical, parallel, perpendicular, equal, tangent, collinear, distance and fixed relations are applied by clicking geometry. Every one of them is its own button in the constraint block beyond the sketch bar's divider — nothing behind a dropdown — and arming one raises a panel that says what to pick next, explains a pick it cannot use, and lists the relations the sketch is holding, each releasable from the list.
