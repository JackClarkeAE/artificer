...
- **Sketch constraints from the canvas.** Coincident, horizontal, vertical, parallel, perpendicular, equal, tangent, collinear, distance and fixed relations are applied by clicking geometry. Every one of them carries its own name on its own tile, in the constraint block beyond the sketch bar's divider — the same tile a drawing tool gets, nothing behind a dropdown — and arming one raises a panel that says what to pick next, explains a pick it cannot use, and lists the relations the sketch is holding, each releasable from the list.
- **Dimensions between two things, and shapes that move as shapes.** The dimension tool takes a second pick: a point and another point for their separation, or a point and an edge for the perpendicular offset between them — which is how a hole is placed in a plate, measured from the sides it has to clear. The value arrives holding what the sketch already showed and is retyped in the panel to drive the geometry to it, with the reference staying where it is. A recipe now moves as a body: dimensioning to a rectangle's edge translates the rectangle rather than shearing it, and a relation over the points of one shape is refused by name, so a sketch cannot be quietly pulled out of shape.
