...
- **Shell.** `shell(open: faces(">Z"), wall: 3)` hollows a prismatic body to one uniform wall, open at one face, at two opposite faces, or closed with a void. It is built from the kernel's own mitred loop offset, exact face cut and Boolean ladder, so the answers are closed-form: a shelled box open at the top has volume `bdh − (b−2w)(d−2w)(h−w)`, a cylinder becomes an annular cup, and `probe.min_wall` reads the wall back.
...
| `shell(open:, wall:)` | Hollows the current body to one wall, open at one face, two opposite faces, or none. |
...
- Primitives, planar-profile extrusion with holes and islands, revolve, drafted extrusion as an exact loft to the profile's offset section, push/pull, holes, ribs, shell, exact mirror, and circular and linear patterns of face features.
